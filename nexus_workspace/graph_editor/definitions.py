    ...
    def register(self, definition: NodeDefinition):
        ...
        if existing is not None:
            ...
            if existing_source != new_source:
                print(
                    f"[GraphEditor] Skipping duplicate node type '{definition.type_id}' "
                    f"from {new_source}; already provided by {existing_source}"
                )
            return existing
...
def register_node_definition_dir(path):
    candidate = Path(path).resolve()
    if candidate.exists() and candidate.is_dir() and candidate not in _ADDITIONAL_NODE_DEFINITION_DIRS:
        _ADDITIONAL_NODE_DEFINITION_DIRS.append(candidate)
    return candidate
...
def load_external_node_definitions(clear_existing: bool = True):
    ...
    for extra_dir in list(_ADDITIONAL_NODE_DEFINITION_DIRS):
        if extra_dir.exists() and extra_dir.is_dir():
            NODE_DEFINITION_LOADER.load_directory(extra_dir, recursive=True)
