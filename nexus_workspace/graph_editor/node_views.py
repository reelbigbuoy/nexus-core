...
@dataclass
class NodeConnectionNodeTypeRule:
    source_type_id: str = "*"
    target_type_id: str = "*"
    connection_kind: str = "*"
...
@dataclass
class RequiredNodeSpec:
    type_id: str
    title: str = ""
    x: float = 0.0
    y: float = 0.0
    locked: bool = True
    hide_from_palette: bool = True
...
class NodeViewRules:
    ...
    allowed_connection_node_type_rules: List[NodeConnectionNodeTypeRule] = field(default_factory=list)
    blocked_connection_node_type_rules: List[NodeConnectionNodeTypeRule] = field(default_factory=list)
    required_nodes: List[RequiredNodeSpec] = field(default_factory=list)
...
class NodeViewLoader:
    ...
    def load_file(self, path: Union[str, Path]) -> NodeViewDefinition:
        ...
        definition = NodeViewDefinition(
            ...
            rules=NodeViewRules(
                ...
                allowed_connection_node_type_rules=[self._parse_node_type_rule(item) for item in list(rules_data.get("allowed_connection_node_type_rules", []) or [])],
                blocked_connection_node_type_rules=[self._parse_node_type_rule(item) for item in list(rules_data.get("blocked_connection_node_type_rules", []) or [])],
                required_nodes=[self._parse_required_node(item) for item in list(rules_data.get("required_nodes", []) or [])],
    ...
    @staticmethod
    def _parse_node_type_rule(data):
        data = data or {}
        return NodeConnectionNodeTypeRule(
            source_type_id=str(data.get("source_type_id", "*") or "*"),
            target_type_id=str(data.get("target_type_id", "*") or "*"),
            connection_kind=str(data.get("connection_kind", "*") or "*"),
        )

    @staticmethod
    def _parse_required_node(data):
        data = data or {}
        return RequiredNodeSpec(
            type_id=str(data.get("type_id", "") or ""),
            title=str(data.get("title", "") or ""),
            x=float(data.get("x", 0.0) or 0.0),
            y=float(data.get("y", 0.0) or 0.0),
            locked=bool(data.get("locked", True)),
            hide_from_palette=bool(data.get("hide_from_palette", True)),
        )
...
class FilteredNodeDefinitionRegistry(NodeDefinitionRegistry):
    def __init__(self, base_registry: NodeDefinitionRegistry, definitions: Sequence[NodeDefinition], view_definition: Optional[NodeViewDefinition] = None):
        ...
        self._palette_hidden_type_ids = set()
        rules = getattr(view_definition, 'rules', None) if view_definition is not None else None
        for spec in list(getattr(rules, 'required_nodes', []) or []):
            if getattr(spec, 'hide_from_palette', False) and getattr(spec, 'type_id', None):
                self._palette_hidden_type_ids.add(spec.type_id)
    ...
    def _is_palette_visible(self, definition):
        if definition is None:
            return False
        if definition.type_id in self._palette_hidden_type_ids:
            return False
    ...
    def all_definitions(self):
        return [definition for definition in super().all_definitions() if self._is_palette_visible(definition)]
    ...
    def search(self, query: str):
        return [definition for definition in super().search(query) if self._is_palette_visible(definition)]


class NodeViewSession:
    def __init__(self, base_registry: NodeDefinitionRegistry, view_registry: NodeViewRegistry, select_default_on_reset: bool = False, allowed_view_ids=None, allowed_view_prefixes=None):
        self._base_registry = base_registry
        self._view_registry = view_registry
        self._select_default_on_reset = bool(select_default_on_reset)
        self._allowed_view_ids = {str(item) for item in (allowed_view_ids or []) if str(item)}
        self._allowed_view_prefixes = tuple(str(item) for item in (allowed_view_prefixes or []) if str(item))
    ...
    def _view_is_allowed(self, view_definition: Optional[NodeViewDefinition]) -> bool:
        if view_definition is None:
            return False
        if not self._allowed_view_ids and not self._allowed_view_prefixes:
            return True
        view_id = str(view_definition.view_id or "")
        return view_id in self._allowed_view_ids or any(view_id.startswith(prefix) for prefix in self._allowed_view_prefixes)

    def available_views(self) -> List[NodeViewDefinition]:
        return [view for view in self._view_registry.all_views() if self._view_is_allowed(view)]

    def active_view_id(self) -> Optional[str]:
        return self._active_view_id

    def active_view(self) -> Optional[NodeViewDefinition]:
        view = self._view_registry.get(self._active_view_id) if self._active_view_id else None
        return view if self._view_is_allowed(view) else None
    ...
    def set_active_view_id(self, view_id: Optional[str]) -> Optional[NodeViewDefinition]:
        ...
        if view_definition is not None and not self._view_is_allowed(view_definition):
...
# Plugin-owned graph/view manifests are registered by plugin adapters. This keeps
# the framework generic while still allowing plugin-defined graph types and menu
# filtering to participate in the shared view registry.
_ADDITIONAL_NODE_VIEW_MANIFEST_DIRS = []


def register_node_view_manifest_dir(path):
    candidate = Path(path).resolve()
    if candidate.exists() and candidate.is_dir() and candidate not in _ADDITIONAL_NODE_VIEW_MANIFEST_DIRS:
...
def load_node_views(clear_existing=True):
    ...
    for extra_dir in list(_ADDITIONAL_NODE_VIEW_MANIFEST_DIRS):
        if extra_dir.exists() and extra_dir.is_dir():
            NODE_VIEW_LOADER.load_directory(extra_dir, recursive=True)
